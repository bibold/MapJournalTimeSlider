define([
    ...
    return declare(Memory, {
        ...
        arrays2hash: function(keys,values){
            var obj = {};
            var len = keys.length;
            for (i = 0; i < len; i++) {
                obj[keys[i]] = values[i];
            ...
            for (var i = 0, numlines = lines.length; i < numlines; i++) {
                ...
                for (var j = 0, numparts = parts.length; j < numparts; j++) {
                    ...
                    if (numquotes % 2 === 0) {
                        ...
                        if (numquotes) {
                            var match = this.quoteRx.exec(value);
                            if (match) {
                                // Good quoted string; unescape any quotes within.
                                values.push(match[1].replace(this.doubleQuoteRx, '"'));
                            }
                            else {
                ...
                if (!numquotes) {
                    ...
                    if (!fieldNames) {
                        ...
                        var item = this.arrays2hash(fieldNames, values);
            ...
            for (var i = -1; i < data.length; i++) {
                ...
                for (var j = 0; j < fieldNames.length; j++) {
                    ...
                    output += (j > 0 ? delimiter : '') +
                        (needsQuotes ? '"' + value.replace(this.singleQuoteRx, '""') + '"' : value);
                }
            }
